...
describe('writeIssueCommand', () => {
  it('checks file ignore rules in the selected WSL distro', async () => {
    ...
    await writeIssueCommand(TEST_REPO_PATH, TEST_ORCA_DIR_NAME, 'local command', {
      wslDistro: 'Ubuntu'
    })
  ...
  it('deletes the local override when the override is cleared', async () => {
    const { writeIssueCommand } = await import('./issue-command-file')
    const fs = await import('node:fs')
    await writeIssueCommand(TEST_REPO_PATH, TEST_ORCA_DIR_NAME, '   ')
  ...
  it('writes into the configured directory name and ignores that name', async () => {
    ...
    const { writeIssueCommand } = await import('./issue-command-file')
    await writeIssueCommand(TEST_REPO_PATH, TEST_CUSTOM_ORCA_DIR_NAME, 'local command')
