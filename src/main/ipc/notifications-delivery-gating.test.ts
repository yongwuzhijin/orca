...
describe('registerNotificationHandlers', () => {
  ...
  it('opens the current macOS app notification settings entry', async () => {
    ...
    process.env.ORCA_DEV_MACOS_BUNDLE_ID = 'com.yongwuzhijin.dmonwork.dev.fb5a47066f08'
    try {
      registerNotificationHandlers({
        getSettings: () => ({
          notifications: {
      ...
      expect(shellOpenExternalMock).toHaveBeenCalledWith(
        'x-apple.systempreferences:com.apple.Notifications-Settings.extension?id=com.yongwuzhijin.dmonwork.dev.fb5a47066f08'
      )
    } finally {
