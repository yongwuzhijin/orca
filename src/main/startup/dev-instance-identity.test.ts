...
describe('dev-instance-identity', () => {
  it('keeps packaged identity stable', () => {
    expect(getDevInstanceIdentity(false, {})).toMatchObject({
      ...
      devLabel: null,
      dockBadgeLabel: null,
      appUserModelId: 'com.yongwuzhijin.dmonwork'
    })
  })
  ...
  it('pins a stable dev appName across branches so the safeStorage key does not churn', () => {
    ...
    expect(a.appName).toBe('DmonWork Dev')
    expect(b.appName).toBe('DmonWork Dev')
    expect(a.appName).not.toBe('DmonWork')
  })
  ...
  it('never renames a packaged build before ready', () => {
    ...
    // a pre-ready rename would repoint forks ("Orca ALab Edition") at DmonWork's key.
  ...
  it('derives a readable dev label from worktree and branch env', () => {
    ...
    expect(identity.name).toBe('DmonWork: nwparker/dev-indicator')
    expect(identity.dockBadgeLabel).toBeNull()
    expect(identity.appUserModelId).toMatch(/^com\.yongwuzhijin\.dmonwork\.dev\.[a-f0-9]{10}$/)
  })
  ...
  it('includes the branch when it differs from the worktree basename', () => {
    ...
    expect(identity.name).toBe('DmonWork: feature/billing-shell')
    expect(identity.dockBadgeLabel).toBeNull()
  })
  ...
  it('allows an explicit label override', () => {
    ...
    expect(identity.name).toBe('DmonWork: feature/other')
    expect(identity.dockBadgeLabel).toBeNull()
  })
})
