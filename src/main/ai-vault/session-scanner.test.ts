...
describe('scanAiVaultSessions', () => {
  ...
  it('indexes every supported agent transcript format with native resume commands', async () => {
    ...
    await writeFile(
      join(roots.cursorProjectsDir, 'project', '.workspace-trusted'),
      JSON.stringify({ workspacePath: '/tmp/cursor' })
    )
    await writeFile(
    ...
    expect(commandByAgent.get('cursor')).toBe(
      "cd '/tmp/cursor' && cursor-agent --resume 'cursor-session'"
    )
