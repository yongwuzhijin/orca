...
function createWebPreloadApi(): Partial<PreloadApi> {
  ...
  return {
    ...
    agentStatus: {
      ...
      reconcileEndedProcess: (_paneKey: string) => {},
...
function createRuntimeEnvironmentsApi(): NonNullable<Partial<PreloadApi>['runtimeEnvironments']> {
  return {
    ...
    prepareBrowserClientHostPlacement: async () => ({ kind: 'server' as const }),
...
function createSkillsApi(): NonNullable<Partial<PreloadApi>['skills']> {
  return {
    ...
    previewDelete: async () => ({ operationId: 'unsupported', skills: [] }),
    delete: async () => ({ operationId: 'unsupported', skills: [] }),
    onUpdateRun: () => () => {}
  }
}
...
function mergeHostWebUIState(local: PersistedUIState, incoming: PairedUiState): PersistedUIState {
  ...
  const pinned = {
    automationHostFilter: undefined,
